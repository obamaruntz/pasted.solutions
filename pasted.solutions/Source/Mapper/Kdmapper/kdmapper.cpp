...
uint64_t kdmapper::AllocMdlMemory(HANDLE iqvw64e_device_handle, uint64_t size, uint64_t* mdlPtr) {
	...
	if (!mdl) {
		FuckOff(L"[-] Can't allocate pages for mdl" << std::endl);
		return { 0 };
	}

	uint32_t byteCount = 0;
	if (!intel_driver::ReadMemory(iqvw64e_device_handle, mdl + 0x028 /*_MDL : byteCount*/, &byteCount, sizeof(uint32_t))) {
		FuckOff(L"[-] Can't read the _MDL : byteCount" << std::endl);
		return { 0 };
	}

	if (byteCount < size) {
		FuckOff(L"[-] Couldn't allocate enough memory, cleaning up" << std::endl);
	...
	if (!mappingStartAddress) {
		FuckOff(L"[-] Can't set mdl pages cache, cleaning up." << std::endl);
		intel_driver::MmFreePagesFromMdl(iqvw64e_device_handle, mdl);
		intel_driver::FreePool(iqvw64e_device_handle, mdl);
		return { 0 };
	}
	...
	if (!result) {
		FuckOff(L"[-] Can't change protection for mdl pages, cleaning up" << std::endl);
		intel_driver::MmUnmapLockedPages(iqvw64e_device_handle, mappingStartAddress, mdl);
		intel_driver::MmFreePagesFromMdl(iqvw64e_device_handle, mdl);
		intel_driver::FreePool(iqvw64e_device_handle, mdl);
		return { 0 };
	}
	FuckOff(L"[+] Allocated pages for mdl" << std::endl);
...
uint64_t kdmapper::AllocIndependentPages(HANDLE device_handle, uint32_t size)
{
	...
	if (!base)
	{
		FuckOff(L"[-] Error allocating independent pages" << std::endl);
		return 0;
	}

	if (!intel_driver::MmSetPageProtection(device_handle, base, size, PAGE_EXECUTE_READWRITE))
	{
		FuckOff(L"[-] Failed to change page protections" << std::endl);
...
uint64_t kdmapper::MapDriver(HANDLE iqvw64e_device_handle, BYTE* data, ULONG64 param1, ULONG64 param2, bool free, bool destroyHeader, AllocationMode mode, bool PassAllocationAddressAsFirstParam, mapCallback callback, NTSTATUS* exitCode) {
	...
	if (!nt_headers) {
		FuckOff(L"[-] Invalid format of PE image" << std::endl);
		return 0;
	}

	if (nt_headers->OptionalHeader.Magic != IMAGE_NT_OPTIONAL_HDR64_MAGIC) {
		FuckOff(L"[-] Image is not 64 bit" << std::endl);
		return 0;
	}
	...
	if (!kernel_image_base) {
		FuckOff(L"[-] Failed to allocate remote image in kernel" << std::endl);
	...
	do {
		FuckOff(L"[+] Image base has been allocated at 0x" << reinterpret_cast<void*>(kernel_image_base) << std::endl);
		...
		if (destroyHeader) {
			...
			FuckOff(L"[+] Skipped 0x" << std::hex << TotalVirtualHeaderSize << L" bytes of PE Header" << std::endl);
		}
		...
		{
			FuckOff(L"[-] Failed to fix cookie" << std::endl);
			return 0;
		}

		if (!ResolveImports(iqvw64e_device_handle, portable_executable::GetImports(local_image_base))) {
			FuckOff(L"[-] Failed to resolve imports" << std::endl);
		...
		if (!intel_driver::WriteMemory(iqvw64e_device_handle, realBase, (PVOID)((uintptr_t)local_image_base + (destroyHeader ? TotalVirtualHeaderSize : 0)), image_size)) {
			FuckOff(L"[-] Failed to write local image to remote image" << std::endl);
			kernel_image_base = realBase;
			break;
		}
		...
		FuckOff(L"[<] Calling DriverEntry 0x" << reinterpret_cast<void*>(address_of_entry_point) << std::endl);

		if (callback) {
			if (!callback(&param1, &param2, realBase, image_size, mdlptr)) {
				FuckOff(L"[-] Callback returns false, failed!" << std::endl);
				kernel_image_base = realBase;
				break;
			}
		}
		...
		if (!intel_driver::CallKernelFunction(iqvw64e_device_handle, &status, address_of_entry_point, (PassAllocationAddressAsFirstParam ? realBase : param1), param2)) {
			FuckOff(L"[-] Failed to call driver entry" << std::endl);
			kernel_image_base = realBase;
			break;
		}
		...
		FuckOff(L"[+] DriverEntry returned 0x" << std::hex << status << std::endl);

		// Free memory
		if (free) {
			FuckOff(L"[+] Freeing memory" << std::endl);
			...
			if (free_status) {
				FuckOff(L"[+] Memory has been released" << std::endl);
			}
			else {
				FuckOff(L"[-] WARNING: Failed to free memory!" << std::endl);
			}
		}
...
bool kdmapper::FixSecurityCookie(void* local_image, uint64_t kernel_image_base)
{
	...
	{
		FuckOff(L"[+] Load config directory wasn't found, probably StackCookie not defined, fix cookie skipped" << std::endl);
		return true;
	}
	...
	if (!stack_cookie)
	{
		FuckOff(L"[+] StackCookie not defined, fix cookie skipped" << std::endl);
	...
	if (*(uintptr_t*)(stack_cookie) != 0x2B992DDFA232) {
		FuckOff(L"[-] StackCookie already fixed!? this probably wrong" << std::endl);
		return false;
	}

	FuckOff(L"[+] Fixing stack cookie" << std::endl);
