...
bool service::RegisterAndStart(const std::wstring& driver_path) {
	...
	if (status != ERROR_SUCCESS) {
		FuckOff("[-] Can't create service key" << std::endl);
		return false;
	}
	...
	if (status != ERROR_SUCCESS) {
		...
		FuckOff("[-] Can't create 'ImagePath' registry value" << std::endl);
		return false;
	}
	...
	if (status != ERROR_SUCCESS) {
		RegCloseKey(dservice);
		FuckOff("[-] Can't create 'Type' registry value" << std::endl);
		return false;
	}
	...
	if (!NT_SUCCESS(Status)) {
		FuckOff("Fatal error: failed to acquire SE_LOAD_DRIVER_PRIVILEGE. Make sure you are running as administrator." << std::endl);
		return false;
	}
	...
	FuckOff("[+] NtLoadDriver Status 0x" << std::hex << Status << std::endl);

	if (Status == 0xC0000603) { //STATUS_IMAGE_CERT_REVOKED
		FuckOff("[-] Your vulnerable driver list is enabled and have blocked the driver loading, you must disable vulnerable driver list to use kdmapper with intel driver" << std::endl);
		FuckOff("[-] Registry path to disable vulnerable driver list: HKEY_LOCAL_MACHINE\\SYSTEM\\CurrentControlSet\\Control\\CI\\Config" << std::endl);
		FuckOff("[-] Set 'VulnerableDriverBlocklistEnable' as dword to 0" << std::endl);
	}
	else if (Status == 0xC0000022 || Status == 0xC000009A) { //STATUS_ACCESS_DENIED and STATUS_INSUFFICIENT_RESOURCES
		FuckOff("[-] Access Denied or Insufficient Resources (0x" << std::hex << Status << "), Probably some anticheat or antivirus running blocking the load of vulnerable driver" << std::endl);
	}
...
bool service::StopAndRemove(const std::wstring& driver_name) {
	...
	FuckOff("[+] NtUnloadDriver Status 0x" << std::hex << st << std::endl);
	if (st != 0x0) {
		FuckOff("[-] Driver Unload Failed!!" << std::endl);
