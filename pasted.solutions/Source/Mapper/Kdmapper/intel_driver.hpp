...
namespace intel_driver
{
	...
	bool CallKernelFunction(HANDLE device_handle, T* out_result, uint64_t kernel_function_address, const A ...arguments) {
		...
		if (ntdll == 0) {
			FuckOff(L"[-] Failed to load ntdll.dll" << std::endl); //never should happens
			return false;
		}
		...
		if (!NtAddAtom)
		{
			FuckOff(L"[-] Failed to get export ntdll.NtAddAtom" << std::endl);
			return false;
		}
		...
		*(uint64_t*)&kernel_injected_jmp[2] = kernel_function_address;
		...
		if (!kernel_NtAddAtom) {
			FuckOff(L"[-] Failed to get export ntoskrnl.NtAddAtom" << std::endl);
			return false;
		}
		...
		if (original_kernel_function[0] == kernel_injected_jmp[0] &&
			...
			FuckOff(L"[-] FAILED!: The code was already hooked!! another instance of kdmapper running?!" << std::endl);
			return false;
		}
